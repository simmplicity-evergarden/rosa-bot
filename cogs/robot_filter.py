...
class Inflatables_Filter_Cog(commands.Cog):
	...
	async def on_message(self, message):
		...
		if dice_roll < config.getint('numbers','inflatable_squeak_chance'):
			...
			wm_contents = self.robot_modifier(message.content, 0)
	...
	# Modify messages based on user's corruption level
	def robot_modifier(self, message_content: str, corruption_level: int):
		# All messages
		# Convert capitalization - first letter caps, rest is lower
		new_message = message_content.capitalize()
